...
PID_FILE="$INSTALLDIR/var/log/.cron.pid"
CRON_ARGS=( "na" )
...
for ARGS in "${CRON_ARGS[@]}"; do
  echo "$PHP_BIN $REL_INSTALLDIR/cron.php $ARGS" | fakechroot /usr/sbin/chroot /microcloud/domains/$DOMAIN_GROUP /bin/bash >> $LOG_FILE 2>&1
  [ $RES -ne 0 ] && RES=$?
done
