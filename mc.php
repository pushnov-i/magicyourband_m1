...
* @link    	http://joom.ru
* @version 	002
* @download 	http://freephpscripts.ru
*/

$JSYS_FILE = basename($_SERVER['SCRIPT_FILENAME']);

//пароль для входа (для авторизации нужны включенные cookies)
$JSYS_PASSWORD    = 'vicvic';
...
class JSYSOPERATION_Login {
	...
	function DoLogin() {
		...
		global $JSYS_PASSWORD, $COOKIE_NAME, $JSYS_FILE;

		$pass = JSYS_Utils::getParam('pass','','string',false);
		if ($pass!=$JSYS_PASSWORD) {
		
			echo "Ошибка: Вы ввели не правильный пароль<br>";
			echo "<br><a href='$JSYS_FILE?action=Login&task=main'>Вернуться к форме входа</a>";
			return;
		}
...
class JSYSOPERATION_FileView {
	...
	function viewDir() {
	    ...
	    for ($i=0,$start = $outdir_a[0];$i<sizeof($outdir_a)-1;$i++) {
	    	...
	    	$outdir = str_replace($di,"<a href='$JSYS_FILE?action=FileView&DIR=$dlink'>".$di."</a>",$outdir);
	    }
	    
		echo "<p align=center>Содержание директории <b>{$this->base}{$outdir}</b></p>
		<form action='$JSYS_FILE?action=FileView' method=post>
		...
		$myuid = getmyuid();
		if ($DIR!='/') echo "<tr><td width=20></td><td width=80%><a href='$JSYS_FILE?action=FileView&DIR=".urlencode($parent)."'><b>..</b></a></td><td width=18%></td><td></td><td></td></tr>";
		...
		foreach($files as $file) {
			...
			if (is_dir($file)) {
				...
				$name = "<a href='$JSYS_FILE?action=FileView&DIR=".urlencode($DIR.$bname.'/')."'><b>$bname</b></a>";
				$stat = '';
			}
			else {
				...
				$name = "<a href='$JSYS_FILE?action=FileView&task=ViewFile&File=".urlencode($file)."&DIR=".urlencode($DIR)."'>$rname</a>";

				global $JSYS_SHOW_DOWNLOAD_LINK;
				if ($JSYS_SHOW_DOWNLOAD_LINK) $name .= "&nbsp;<a href='$JSYS_FILE?action=FileView&task=ReadFile&NO_TEMPLATE=1&src=".urlencode($file)."'>+</a>";
	...
	function ViewFile() {
		...
		switch($ext) {
			...
			case 'jpeg':
			case 'png':
				echo "<p style='text-align: center; background: #ffffff; padding:15px'><img src='$JSYS_FILE?action=FileView&task=ViewImage&NO_TEMPLATE=1&src=".urlencode($file)."'></p>";
			...
			default:
				...
				<form action='$JSYS_FILE?action=FileView&task=SaveFile' method=post>
	...
	function SaveFile() {
		...
		echo "<br><a href='$JSYS_FILE?action=FileView&DIR=".urlencode($DIR)."'>Вернуться к директории</a>";
	}
	...
	function DoChmod() {
		...
		echo "<br>Операции выполнены. <a href='$JSYS_FILE?action=FileView&DIR=".urlencode($DIR)."'>Вернуться к директории</a>";
	}
...
class JSYS_Utils {
	...
	static function getParam($name,$default,$type,$addsl = true)
	{
		$arr = $_REQUEST;
	...
	function link($title,$path) {
		global $action, $JSYS_FILE;
		return "<a href='$JSYS_FILE?action=$action&task=$path'>$title</a>";
	}
}
...
class JSYS_Form {
	...
	function JSYS_Form($task,$title) {
		global $action, $JSYS_FILE;
		$this->content="<p align=center>$title</p><form action='$JSYS_FILE?action=$action&task=$task' method='post'><table align=center width=80%>";
		$this->cancel = false;
	}
...
class JSYS_Template {
	...
	function out() {
		global $action, $task, $JSYS_FILE;

		$tpl = <<<TEMPLATE
<html>
<head>
<title>$JSYS_FILE</title>
<style>
input,select {
...
<tr>
	...
	<a href="$JSYS_FILE?action=Main">Начало</a> <font color="#8daa94">/</font>
	<a href="$JSYS_FILE?action=SetPassword">Установить пароль</a> <font color="#8daa94">/</font>
	<a href="$JSYS_FILE?action=Info">Информация</a> <font color="#8daa94">/</font>
	<a href="$JSYS_FILE?action=FileView">Файл Менеджер</a> <font color="#8daa94">/</font> 
	<a href="$JSYS_FILE?action=Login&task=Logout">Выйти из системы</a>  
