...
class Ebizmarts_SagePaySuite_PaymentController extends Mage_Core_Controller_Front_Action {
    ...
    public function _MW_OSCSaveBilling()
    {
        $addressData = $_SESSION['billing_shipping_sagepay'];
        unset($_SESSION['billing_shipping_sagepay']);

        $this->getOnepage()->saveBilling($addressData['billing'], $addressData['billing']['billing_address_id']);
        $this->getOnepage()->saveShipping($addressData['shipping'], $addressData['shipping']['shipping_address_id']);

        $this->getOnepage()->getQuote()->setData('customer_email', $addressData['billing']['email']);
        $this->getOnepage()->getQuote()->setData('customer_firstname', $addressData['billing']['firstname']);
        $this->getOnepage()->getQuote()->setData('customer_lastname', $addressData['billing']['lastname']);

        // Get payment method
        if(isset($addressData['payment']['method']) && $addressData['payment']['method'] != '') {
        	$paymentMethod = $addressData['payment']['method'];
        } else {
        	$paymentMethod = null;
        }
        // Save payment method
        if ($this->getOnepage()->getQuote()->isVirtual()) {
            $this->getOnepage()->getQuote()->getBillingAddress()->setPaymentMethod($paymentMethod);
        } else {
            $this->getOnepage()->getQuote()->getShippingAddress()->setPaymentMethod($paymentMethod);
        }

        // Get shipping method
        if(isset($addressData['shipping_method']) && $addressData['shipping_method'] != '') {
        	$shippingMethod = $addressData['shipping_method'];
        } else {
        	$shippingMethod = null;
        }
        ...
        if (!Mage::helper('customer')->isLoggedIn()) {
            if (!empty($addressData['billing']['customer_password']) && !empty($addressData['billing']['confirm_password']) && ($addressData['billing']['customer_password'] == $addressData['billing']['confirm_password'])) {
                $password = $addressData['billing']['customer_password'];
                $this->getOnepage()->getQuote()->setCheckoutMethod('register');
                $this->getOnepage()->getQuote()->getCustomer()->setData('password', $password);
                $this->getOnepage()->getQuote()->setData('password_hash', Mage::getModel('customer/customer')->encryptPassword($password));
            }
    ...
    public function onepageSaveOrderAction() {
        ...
        if ((FALSE === strstr(parse_url($this->_getRefererUrl(), PHP_URL_PATH), 'onestepcheckout')) && is_null($this->getRequest()->getPost('billing'))) { // Not OSC, OSC validates T&C with JS and has it own T&C
            ...
            # Validate checkout Terms and Conditions
            if (FALSE !== Mage::getConfig()->getNode('modules/MW_Onestepcheckout')) {
                $this->_MW_OSCSaveBilling();
            }
        } else {
