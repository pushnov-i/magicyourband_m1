...
class Elsner_Multicurrency_Model_Observer
{
    public function setConfig(Varien_Event_Observer $observer)
    {
        ...
        $ppMerchant = Mage::helper('multicurrency')->getConfig('business_account');
    ...
    public function setPaymentInfo(Varien_Event_Observer $observer)
    {
        ...
        if (in_array($code, array('paypal_standard'))) {
            $payment->setAdditionalInformation('payment_currency', Mage::helper('multicurrency')->getToCurrency());
            $payment->setAdditionalInformation('due_amount', Mage::helper('multicurrency')->convertAmount($order->getBaseGrandTotal()));
            $payment->setAdditionalInformation('exchange_rate', Mage::helper('multicurrency')->getCurrentExchangeRate());
        }
        $payment->save();
    }
