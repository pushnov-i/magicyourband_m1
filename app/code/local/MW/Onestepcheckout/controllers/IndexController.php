<?php
    ...
    class MW_Onestepcheckout_IndexController extends Mage_Checkout_OnepageController
    {
        ...
        public function saveAction()
        {
            ...
            if($post) {
                ...
                if(isset($updates->updatepaymentmethod)) {
                    ...
                    $paymentFreeActive = Mage::getStoreConfig('payment/free/active');
                    if($grandTotal == 0 && $paymentFreeActive == '1') {
                        $data = array();
                        $data['method'] = 'free';
                    }
                    try {
        ...
        public function updateordermethodAction()
        {
            ...
            $paymentFreeActive = Mage::getStoreConfig('payment/free/active');
            if($grandTotal == 0 && $paymentFreeActive == '1') {
                $data_savepayment = array();
                $data_savepayment['method'] = 'free';
            }
            if(empty($data_savepayment)) {
                $data_savepayment['method'] = '';
            }
            try {
